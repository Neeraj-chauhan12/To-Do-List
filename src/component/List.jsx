...
function handleclk(i) {
 ...
 if(maint.length>0){
  ...
  return (
    <>
      <div className='flex flex-col w-screen px-5 overflow-x-hidden gap-4 min-h-screen bg-red-200' >
        <form onSubmit={handlebtn} className='bg-gray-400 w-full px-6 py-5  flex flex-col gap-5 md:justify-between items-center  '>
        <input value={data} required onChange={(e)=>{setdata(e.target.value)}}  className='text-3xl placeholder:text-3xl border-2 w-80 px-2 py-3  rounded border-black' type="text" placeholder='enter the task...' />
        <input value={disc} required onChange={(e)=>{setdisc(e.target.value)}} className='text-3xl placeholder:text-3xl border-2 w-80 px-2 py-3 rounded border-black' type="text" placeholder='enter the disc...' />
